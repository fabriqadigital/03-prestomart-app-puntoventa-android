...
data class WhatsappLinkResult(val url: String, val expiresAt: String?)
...
class ReceiptDeliveryApiDataSource(context: Context) {
    ...
    fun requestWhatsappLink(receiptNumber: String, customerName: String, pdf: File): Result<WhatsappLinkResult> = runCatching {
        ...
        val request = Request.Builder()
            .url(resolver.endpoint(ApiConfig.RECEIPT_SEND_WHATSAPP))
            .post(body)
            ...
            if (!response.isSuccessful || json?.optBoolean("success", false) != true) {
                throw Exception(
                    json?.optString("message")?.takeIf { it.isNotBlank() }
                        ?: "No se pudo generar el enlace de WhatsApp (${response.code}).",
                )
            }
            val result = json.optJSONObject("result")
            val url = result?.optString("url").orEmpty()
            if (url.isBlank()) throw Exception("El servidor no devolvió un enlace válido.")
            WhatsappLinkResult(url, result?.optString("expires_at")?.takeIf { it.isNotBlank() })
        }
    }
